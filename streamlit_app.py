...
def load_and_preprocess_data():
    ...
    try:
        ...
        if 'OFFER DEPT' in df.columns:
            ...
            df['OFFER DEPT'] = df['OFFER DEPT'].str.replace('Business and Econom', 'Business & Economics', regex=False)
            
            # Replace "Faculty of ..." with "... Faculty"
            df['OFFER DEPT'] = df['OFFER DEPT'].str.replace('Faculty of ', '', regex=False)
            df['OFFER DEPT'] = df['OFFER DEPT'].apply(lambda x: x + ' Faculty' if x and not x.endswith('Faculty') else x)
            
            # Replace "and" with "&"
            df['OFFER DEPT'] = df['OFFER DEPT'].str.replace(' and ', ' & ', regex=False)
...
def create_filters_panel(df):
    ...
    if 'CLASS SECTION' in df.columns:
        ...
        class_sections = [section for section in class_sections if section != '' and pd.notna(section) and section != 'nan']
    ...
    if 'ACAD_CAREER' in df.columns:
        ...
        curricula = [cur for cur in curricula if cur != '' and pd.notna(cur) and cur != 'nan']
    ...
    if 'OFFER DEPT' in df.columns:
        ...
        departments = [dept for dept in departments if dept != '' and pd.notna(dept) and dept != 'nan']
    ...
    if 'WEEKDAY' in df.columns:
        ...
        weekdays = [day for day in weekdays if day != '' and pd.notna(day) and day != 'nan']
    ...
    st.sidebar.markdown("---")
    venue_provided = st.sidebar.checkbox("Venue is provided", value=True, key="venue_provided_filter")
    ...
    if 'START TIME' in df.columns or 'END TIME' in df.columns:
        ...
        if all_times:
            ...
            time_options = [t for t in time_options if min_time <= t <= max_time]
    ...
    sort_options = ['COURSE CODE', 'CLASS SECTION', 'OFFER DEPT', 'ACAD_CAREER', 'START DATE', 'WEEKDAY', 'START TIME', 'END TIME']
    available_sort_options = [opt for opt in sort_options if opt in df.columns]
    sort_by = st.sidebar.selectbox("Sort by", available_sort_options, key="sort_by")
    sort_order = st.sidebar.radio("Order", ['Ascending', 'Descending'], key="sort_order")
...
def create_current_classes_filters_panel(df):
    ...
    # Venue is provided checkbox
    st.sidebar.markdown("---")
    venue_provided = st.sidebar.checkbox("Venue is provided", value=True, key="current_venue_provided_filter")
    filters['venue_provided'] = venue_provided
...
def create_upcoming_classes_filters_panel(df):
    ...
    # Venue is provided checkbox
    st.sidebar.markdown("---")
    venue_provided = st.sidebar.checkbox("Venue is provided", value=True, key="upcoming_venue_provided_filter")
    filters['venue_provided'] = venue_provided
...
def apply_filters(df, filters):
    ...
    if 'venue_provided' in filters and filters['venue_provided']:
        filtered_df = filtered_df[(filtered_df['VENUE'] != '') & (filtered_df['VENUE'].notna()) & (filtered_df['VENUE'] != 'nan')]
    
    # Apply date range filter
    if 'date_range' in filters:
        start_date, end_date = filters['date_range']
        # Filter out rows with NaT dates before comparison
        date_filtered = filtered_df[filtered_df['START DATE'].notna() & filtered_df['END DATE'].notna()]
    ...
    if 'time_range' in filters:
        ...
        time_filtered = filtered_df[filtered_df['START TIME'].notna() & filtered_df['END TIME'].notna()]
...
def apply_sorting(df, sort_by, sort_order):
    ...
    if sort_by in ['COURSE CODE', 'CLASS SECTION', 'OFFER DEPT', 'ACAD_CAREER', 'WEEKDAY']:
        ...
        temp_df = df.copy()
        temp_df[f'{sort_by}_sort'] = temp_df[sort_by].replace('', 'zzzz')  # Push empty strings to end
        temp_df = temp_df.sort_values(by=f'{sort_by}_sort', ascending=ascending, na_position='last')
...
def display_current_classes_page(df):
    ...
    sort_options = ['COURSE CODE', 'CLASS SECTION', 'OFFER DEPT', 'ACAD_CAREER', 'START TIME', 'END TIME']
    available_sort_options = [opt for opt in sort_options if opt in df.columns]
    sort_by = st.sidebar.selectbox("Sort by", available_sort_options, key="current_sort_by")
    sort_order = st.sidebar.radio("Order", ['Ascending', 'Descending'], key="current_sort_order")
    ...
    if current_classes.empty:
        ...
        display_df = current_classes.head(st.session_state.current_display_count)
...
def display_upcoming_classes_page(df):
    ...
    sort_options = ['START TIME', 'END TIME', 'COURSE CODE', 'CLASS SECTION', 'OFFER DEPT', 'ACAD_CAREER']
    available_sort_options = [opt for opt in sort_options if opt in df.columns]
    sort_by = st.sidebar.selectbox("Sort by", available_sort_options, key="upcoming_sort_by")
    sort_order = st.sidebar.radio("Order", ['Ascending', 'Descending'], key="upcoming_sort_order")
    ...
    if upcoming_classes.empty:
        ...
        display_df = upcoming_classes.head(st.session_state.upcoming_display_count)
...
def main():
    ...
    if df.empty:
        st.warning("No data loaded. Please check if the Excel file exists in the working directory.")
        return
    
    # Display term information at the top
    if 'TERM' in df.columns and not df.empty:
        term = df['TERM'].iloc[0] if not df['TERM'].isna().all() else "Unknown Term"
    ...
    if page == "All Courses":
        st.markdown("Browse and filter course schedules from the uploaded Excel file.")
